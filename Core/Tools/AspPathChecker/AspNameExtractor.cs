...
public static class AspNameExtractor
{
    ...
    public static List<string> Extract(string raw, out Telemetry telemetry)
    {
        ...
        if (!tsvDetected)
        {
            // Plain list mode (1 name per line)
            names = DedupPreserveOrder(lines, out var dup);
            telemetry = new Telemetry(false, false, 0, lines.Count, names.Count, dup);
            return names;
        }

        // TSV mode
        var headerCols = SplitTsvLine(lines[0]);
        var nameIdx = IndexOfColumn(headerCols, "Name");

        var headerDetected = nameIdx >= 0 && headerCols.Length > 1;
        if (!headerDetected)
        ...
        for (int i = startRow; i < lines.Count; i++)
        {
            ...
            var v = cols[nameIdx].Trim();
            if (string.IsNullOrWhiteSpace(v)) continue;

            collected.Add(v);
        }

        names = DedupPreserveOrder(collected, out var duplicates);
        telemetry = new Telemetry(true, headerDetected, nameIdx, lines.Count, names.Count, duplicates);
        return names;
    }
